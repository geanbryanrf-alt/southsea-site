...
import { ArrowRight, ArrowUpRight, Building2, CheckCircle2, FileCheck2, Landmark, LineChart, Scale, ShieldCheck } from "lucide-react";
...
export function HomeClient() {
  ...
  return (
    <div className="overflow-hidden">
      ...
      <section className="bg-background py-20 lg:py-24"><div className="container mx-auto px-5 md:px-10"><Reveal className="max-w-2xl"><p className="eyebrow">Quando a Southsea entra</p><h2 className="section-title">Decisões de capital pedem contexto, método e coordenação.</h2><p className="section-copy mt-5">A atuação começa quando a empresa precisa transformar uma decisão relevante em uma operação financeiramente viável e executável.</p></Reveal><div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{moments.map(([title, text], index) => <Reveal key={title} delay={index * 0.06} className="border border-white/10 bg-[#101010] p-6"><FileCheck2 className="mb-6 text-primary" size={22} /><h3 className="font-serif text-2xl text-foreground">{title}</h3><p className="mt-3 text-sm leading-6 text-foreground/60">{text}</p></Reveal>)}</div></div></section>

      <section className="relative overflow-hidden border-y border-white/10 bg-[#030303] py-20 lg:py-24"><div className="pointer-events-none absolute left-1/2 top-0 h-72 w-[55rem] -translate-x-1/2 bg-[radial-gradient(ellipse_at_top,rgba(194,142,70,0.09),transparent_68%)]" /><div className="container relative mx-auto px-5 md:px-10"><Reveal className="max-w-4xl"><p className="eyebrow">Soluções</p><h2 className="section-title">Estruturas desenhadas a partir da operação.</h2></Reveal><SolutionsCarousel /></div></section>

      <section className="bg-background py-20 lg:py-24"><div className="container mx-auto px-5 md:px-10"><Reveal className="max-w-2xl"><p className="eyebrow">Modelo de atuação</p><h2 className="section-title">Do enquadramento à execução acompanhada.</h2><p className="section-copy mt-5">A sequência é adaptada ao estágio e à natureza de cada operação, preservando rigor técnico e visibilidade para a decisão.</p></Reveal><div className="mt-12 grid gap-px overflow-hidden border border-white/10 bg-white/10 md:grid-cols-5">{operatingModel.map((step, index) => <Reveal key={step.number} delay={index * 0.05} className="bg-background p-6"><p className="font-serif text-3xl text-primary/80">{step.number}</p><h3 className="mt-9 font-serif text-xl text-foreground">{step.title}</h3><p className="mt-3 text-sm leading-6 text-foreground/60">{step.text}</p></Reveal>)}</div><Link href="/como-atuamos" className="mt-8 inline-flex items-center gap-2 text-sm text-primary hover:text-primary-light">Entender como atuamos <ArrowRight size={16} /></Link></div></section>
      ...
      <section className="bg-background py-20 lg:py-24"><div className="container mx-auto px-5 md:px-10"><Reveal className="border border-primary/30 bg-[linear-gradient(115deg,#111111,#080808_52%,#101010)] p-7 sm:p-10"><p className="eyebrow">Educação executiva</p><div className="grid items-end gap-7 lg:grid-cols-[1.25fr_0.75fr]"><div><h2 className="section-title">O Código Financeiro Corporativo</h2><p className="section-copy mt-5">Conteúdo educacional complementar para quem deseja aprofundar conceitos de capital, estrutura financeira e decisões corporativas. A educação executiva tem identidade própria e não substitui uma análise dedicada.</p></div><Link href="/curso" className="button-secondary justify-center gap-2 border-primary/50 text-primary hover:bg-primary hover:text-[#080808]">Conhecer o programa <ArrowRight size={17} /></Link></div></Reveal></div></section>
